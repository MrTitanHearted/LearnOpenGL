...
BaseMesh BaseModel::processMesh(aiMesh *mesh, const aiScene *scene)
{
    ...
    // std::cout << "[INFO]: Processing mesh: " << mesh->mName.C_Str() << "\n";
    ...
    // std::cout << "[INFO]: Processing mesh: " << mesh->mName.C_Str() << " has completed!\n";
...
{
    // std::cout << "[INFO]: Loading textures with the type: " << typeName << "\n";
    std::vector<MeshTexture> textures;
    for (unsigned int i = 0; i < material->GetTextureCount(type); i++)
    {
        aiString filename;
        material->GetTexture(type, i, &filename);
        // std::cout << "[INFO]: Loading texture: " << filename.C_Str() << "\n";
        bool skip = false;
