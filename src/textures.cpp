...
Texture2D::Texture2D(const char* path, const void* pData, unsigned int size, std::vector<Parameter> params) {
    if (s_Cache.find(path) != s_Cache.end()) {
        m_Id = s_Cache[path].m_Id;
        m_ChannelsCount = s_Cache[path].m_ChannelsCount;
        m_Width = s_Cache[path].m_Width;
        ...
        unsigned char* data = stbi_load_from_memory((const stbi_uc*)pData, size, &m_Width, &m_Height, &m_ChannelsCount, 0);
